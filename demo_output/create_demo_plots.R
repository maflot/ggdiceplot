...
pex1 <- ggplot(toy_data, aes(x=specimen, y=taxon)) +
...
# Demo 3: Large Y axis case
cat("2. Creating largecase plot demo...\n")
# Load dataset sample_dice_data2 from package data folder and assign to toy_data2
load("../data/sample_dice_large.rda")
toy_data <- sample_dice_large
...
pex3 <- ggplot(toy_data, aes(x=specimen, y=taxon)) +
  geom_dice(aes(dots=disease, fill=lfc, size=-log10(q), 
                # Square dims
                width = 0.5, height = 0.5),
            # For missing info
...
ggsave("example_large.png", pex3, width = 10, height = 6, dpi = 300)
