...
GeomDice <- ggplot2::ggproto("GeomDice", ggplot2::Geom,
  ...
  draw_panel = function(data, panel_params, coord, dice_size = 1, dot_size = NULL, 
    ...
    # Always apply boundary constraints, even for explicit dice_size
    x_range <- diff(range(coords$x, na.rm = TRUE))
    y_range <- diff(range(coords$y, na.rm = TRUE))
    
    # Calculate available space per point for boundary validation
    if (x_range > 0 && y_range > 0) {
      ...
      # Calculate maximum safe dice size
      max_safe_size <- min_spacing * 0.4  # 40% of spacing to prevent overlap
      
      # If dice_size is default (1), use auto-scaling
      if (dice_size == 1) {
        dice_size <- max_safe_size
      } else {
        # For explicit dice_size, ensure it doesn't exceed safe boundaries
        dice_size <- min(dice_size, max_safe_size)
      }
      
      # Ensure reasonable bounds - minimum size for visibility
      dice_size <- max(0.05, dice_size)
    }
    
    # Set default dot size if not provided
    if (is.null(dot_size)) {
      # Scale dot size to be visible - use much larger multiplier for small dice
      dot_size <- max(10, dice_size * 60)  # Minimum 10pt size, or 60x dice size for bigger dots
    }
      ...
      if (length(present_categories) > 0) {
        ...
        if (length(valid_categories) > 0) {
          ...
          if (nrow(dot_positions) > 0) {
            ...
            # Use 45% of dice size for dot positioning - increased for larger distances between dots
            dot_positions$x_offset <- dot_positions$x_offset * dice_size * 0.45
            dot_positions$y_offset <- dot_positions$y_offset * dice_size * 0.45
